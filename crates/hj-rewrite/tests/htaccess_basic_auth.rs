...
fn complete_basic_auth_block_builds_a_realm() {
    ...
    .expect("parse");
    let auth = resolve_auth([&ht], "/admin/tool.php")
        .unwrap()
        .expect("realm");
...
fn require_user_lists_feed_the_realm() {
    ...
    .expect("parse");
    let auth = resolve_auth([&ht], "/x").unwrap().expect("realm");
...
fn incomplete_auth_block_keeps_the_deny_collapse() {
    ...
    assert!(
        resolve_auth([&ht], "/admin/tool.php").is_err(),
        "no realm without an AuthUserFile"
    );
    assert!(
        ht.is_forbidden("/admin/tool.php", "tool.php"),
